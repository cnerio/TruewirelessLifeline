...
// error_reporting(E_ALL);   
		if($_SERVER['REQUEST_METHOD']=='POST'){
			...
			if($data['email'] != "" && filter_var($data['email'], FILTER_VALIDATE_EMAIL)){
				...
				$mail->addBCC(MAIL_NOTIFY_DOCS_BCC);
				$mail->isHTML(true);
    ...
    public function read(){
		...
		//if($_POST){
			...
			//print_r($data);
			$camposBase=array("customer_id","first_name","second_name","phone_number","email","dob","city","state","zipcode","order_id","order_status","facility","duplicated","program_benefit","created_at");
			$addWhere="";
			$count=0;
